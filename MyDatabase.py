...
def CreateRelations():
       try:
              ...
              sql="create table if not exists issuebooks(bookcode int, memberno int, doi date,dor date, foreign key (bookcode) references bookrecords(bookcode), foreign key(memberno) references memberrecords(memberno))"
              mycursor.execute(sql)
